...
public:
    ...
    void initialise (const juce::String& commandLine) override
    {
        ...
        // For development: render the window to a PNG and quit ("--snapshot path.png [--add-recorder after] [--select name] [--record] [--panel] [--add-effects]").
        if (const auto args = juce::StringArray::fromTokens (commandLine, true); args.contains ("--snapshot"))
        {
            ...
            if (main != nullptr && args.contains ("--add-effects"))
                main->addDemoEffects();
